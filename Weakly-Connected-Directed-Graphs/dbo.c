...
unsigned int popularidad(const unsigned int node, const unsigned int num_nodes) {
	const unsigned int num_links = (unsigned int) (1.2 * sqrt(num_nodes));
	//numero de links (aristas) es f(node)= ((1-k)/num_nodes) *node + k
	// el maximo es num_links 
	return (unsigned int)(((1.0 - num_links) / num_nodes) * node + num_links);
}
...
int main(int argc, char* argv[]) {
	...
	unsigned int enlace, total_enlaces_salida;
	...
	if (enlaces_a_nodos_no_visitados == NULL) {
		fprintf(stderr, "Error: No se pudo asignar memoria para enlaces_a_nodos_no_visitados.\n");
		exit(EXIT_FAILURE);
	}
	...
	double peso[] = { 1.05 ,1, 1, 1.21, 1.21, 1.21, 1.21, 1.21 };
	...
	for (comm = 0; comm < num_communities; comm++) {
		...
		if (nodos_no_visitados == NULL) {
			fprintf(stderr, "Error: No se pudo asignar memoria para nodos_no_visitados.\n");
			exit(EXIT_FAILURE);
		}
		...
		for (ruta = 0; ruta < 200 + comunity_num_nodes[comm] * 10; ruta++) {
			...
			for (step = 0; step < long_estud_preferen; step++) {
				...
				for (nodo = 0; nodo < comunity_num_nodes[comm]; nodo++) {
					...
					if (visited == false && total_nodos_no_visitados<comunity_num_nodes[comm]) {
				...
				if ((sin_salida_posible == false) && (total_nodos_no_visitados > 0)) {
					...
					enlace = 0;
					bool encontrado_enlace_libre = false;
					...
					if (encontrado_enlace_libre == true) {
						...
						if (next_node == node) {
							printf("error next node=%i ,total_nodes_no_visited %i\n", next_node, total_nodos_no_visitados);
						...
						bool encontrada_conexion_previa = false;
						...
						if (encontrada_conexion_previa == true) {
							...
							printf("error edge already defined\n"); exit(1);
						}
						if (debug) printf(" definiendo com %i node %i edge %i\n", comm, node, enlace);
						...
						if(comm< num_communities && node< comunity_num_nodes[comm])
						for (enlace = 0; enlace < comunity_size_node_links[comm][node]; enlace++) {
						...
						if (total_enlaces_a_nodos_no_visitados != 0) {
							...
							path_length += 1;
							if (path_length >= comunity_num_nodes[comm]) {
								exit(1);
							}
							path_nodes[path_length] = next_node;
							path_length = step + 1;
			...
			for (unsigned int sstep = 0; sstep <= path_length; sstep++) {
				...
				if (sstep > 0) {
					if (path_nodes[sstep] == path_nodes[sstep - 1]) {
						printf("error repeted edge step %i path lenght %i\n", sstep, path_length);
	...
	for (mu_extern_links = 1; mu_extern_links < max_mu_extern_links; mu_extern_links++) { //vamos de 10% 20% 30% ... 60% de enlaces extra
		...
		interconnecta_comunidades_new(num_communities,
			...
			comunidades_otra, comunity_size_node_links_otra, comunity_num_nodes, total_nodes, comm_external_links, total_used_links_at_node, mu_extern_links);
