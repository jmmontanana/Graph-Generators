//Author: J.M. Monta�ana, October 2019
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <limits.h>
...
unsigned int** comunity_size_node_links, unsigned int** comunity_size_node_links_otra) {
	...
	if(comunidades_otra==NULL){	
		tabla_abreviada=true;
	}else	
		tabla_abreviada=false;	
...
const unsigned int node_b, const unsigned int comm_b){
	...
	//por eso (en el caso que sean de la misma comunidad) borramos primero el elemento m�s al final de la cadena, 
	// y despues el m�s cercano al comienzo
	unsigned int mylast;
...
if(local_last>0){// queda algun nodo sin conectarse con source-source_comm 	
		...
		//ahora a�adimos al vector estos dos nuevos enlaces modify_node y modify_dest
...
{ 	
	unsigned int source_comm, dest_pos, dest_comm;
	...
	// if(misma_comunidad==false){
	int contador;
	for(int comm=0;comm<num_communities;comm++){	//this is the community
		contador=0;
		for (int nodo = 0;nodo != comunity_num_nodes[comm];nodo++) {
...
//	}
	...
	while((total_last>1)||(total_links>1)){//at least "last" is 2	
			...
			if(dest_pos==UINT_MAX){//not possible to find dest_pos
				...
				if((vector_com_nodes[source_comm][source_pos]!=vector_com_nodes[dest_comm][dest_pos])||(source_comm!=dest_comm)){ 				
					...
					//a�adimos la conexion source-dest a la vector de comunidades	 	
...
int interconnecta_comunidades_new( const unsigned int num_communities, //constante total de comunidades
	...
	unsigned int*** posible_src_nodes=( unsigned int ***) malloc(num_communities*sizeof( unsigned int **));
	unsigned int*** posible_src_comm=( unsigned int ***) malloc(num_communities*sizeof( unsigned int **));
	unsigned int**  posible_src_total=( unsigned int **)  malloc(num_communities*sizeof( unsigned int *));
	for(int comm_src=0;comm_src<num_communities;comm_src++){
		...
		posible_src_nodes[comm_src]=( unsigned int **) malloc(comunity_num_nodes[comm_src]*sizeof( unsigned int *));
		posible_src_comm[comm_src]=( unsigned int **) malloc(comunity_num_nodes[comm_src]*sizeof( unsigned int *)); 
		posible_src_total[comm_src]=( unsigned int *)  malloc(comunity_num_nodes[comm_src]*sizeof( unsigned int ));
		for(int node=0;node<comunity_num_nodes[comm_src];node++){
			...
			posible_src_nodes[comm_src][node]=( unsigned int *) malloc((total_nodes-comunity_num_nodes[comm_src])*sizeof( unsigned int ));
			posible_src_comm[comm_src][node] =( unsigned int *) malloc((total_nodes-comunity_num_nodes[comm_src])*sizeof( unsigned int ));
		...
		for(int node_src=0;node_src<comunity_num_nodes[comm_src];node_src++){
			...
			posible_src_total[comm_src][node_src]=total_nodes-comunity_num_nodes[comm_src]; 
			...
			for(int comm_dst=0;comm_dst<num_communities;comm_dst++){  
				if(comm_src!=comm_dst){
					for(int node_dest=0;node_dest<comunity_num_nodes[comm_dst];node_dest++){ 
						...
						posible_src_nodes[comm_src][node_src][contador]=node_dest; 
						posible_src_comm[comm_src][node_src][contador]=comm_dst;
						contador++;
					}
				}
	...
	for(int comm_src=0;comm_src<num_communities;comm_src++){
		...
		for(int i=0; i<comunity_num_nodes[comm_src]; i++)
	...
	for(int comm=0;comm<num_communities;comm++){ 
		for(int node=0;node<comunity_num_nodes[comm];node++){
			free(posible_dst_nodes[comm][node]); 
			free(posible_dst_comm[comm][node]); 
			free(posible_src_nodes[comm][node]); 
			free(posible_src_comm[comm][node]); 
		}
		free(posible_dst_nodes[comm]);
		free(posible_dst_comm[comm]);
		free(posible_dst_total[comm]);
		free(posible_src_nodes[comm]);
		free(posible_src_comm[comm]);
		free(posible_src_total[comm]);
	}	
	free(posible_dst_nodes); 
	free(posible_dst_comm); 
	free(posible_dst_total);
	free(posible_src_nodes); 
	free(posible_src_comm); 
	free(posible_src_total);
	return 0;
}
